...
def sample_cells(df, n):
    cells=df.source.values
    cells = [clean_code(cell) for cell in cells]
    ranks = df.pct_rank.values


    if n >= len(cells):
        return [cell[:200] for cell in cells], ranks
    else:
        results = []
        new_ranks=[]
        step = len(cells) / n
        ...
        while int(np.round(idx)) < len(cells):
            ...
            new_ranks.append(ranks[int(np.round(idx))])
            idx += step
        assert cells[0] in results
        if cells[-1] not in results:
            results[-1] = cells[-1]
            new_ranks[-1] = ranks[-1]
        assert len(results)==len(new_ranks)
        return results, new_ranks
...
def get_features(df):
    ...
    for idx, sub_df in tqdm(df.groupby("id")):
        ...
        codes, code_ranks = sample_cells(code_sub_df, n_code)
        features[idx]["total_code"] = total_code
        features[idx]["total_md"] = total_md
        features[idx]["codes"] = codes
        features[idx]["pct_ranks"] = list(code_ranks)
    return features
